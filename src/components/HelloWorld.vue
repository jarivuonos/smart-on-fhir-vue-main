...
export default {
  ...
  computed: {
    authorizeLink() {
      ...
      //return `https://fhir.epic.com/interconnect-fhir-oauth/oauth2/authorize?response_type=code&redirect_uri=${this.redirect}&client_id=${this.clientId}&state=1234&scope=launch, patient.read, patient.search`; // &aud=${this.aud} Epic sandbox, toimii
    },
  },
  async mounted() {
    console.log('redirect: ' + this.redirect);
    console.log('clientId: ' + this.clientId);
    console.log('iss: ' + this.$route.query.iss);
    console.log('launch: ' + this.$route.query.launch);
    console.log('code: ' + this.$route.query.code);
    console.log('params: ' + this.$route.query.iss);
    console.log('----');

 
    this.code = this.$route.query.code; 
    alert('this.code: ' + JSON.stringify(this.code));
